...
function getPrices() {
	...
	axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=${process.env.PREFERRED_CURRENCY}&ids=${process.env.COIN_ID}`).then(res => {
		...
		if(res.data && res.data[0].current_price && res.data[0].price_change_percentage_24h) {
			...
			client.guilds.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${(currentPrice).toLocaleString().replace(/,/g,process.env.THOUSAND_SEPARATOR)}${process.env.CURRENCY_SYMBOL}` + ' | HODL!')
			if(lastPrice > currentPrice) client.user.setStatus('dnd');
			if(lastPrice == currentPrice) client.user.setStatus('idle');
			if(lastPrice < currentPrice) client.user.setStatus('online');
